(function() {
    ...
    var PROVIDER_LABELS={openai:'OpenAI',anthropic:'Anthropic',groq:'Groq',openrouter:'OpenRouter',deepseek:'DeepSeek',glm:'GLM (Z.ai)',kimi:'Kimi',custom:'Custom',ollama:'Ollama'};
    var PROVIDER_ICONS={
    ...
    function fillModelHints(models){
        var dl=document.getElementById('modelHints'); if(!dl)return;
        dl.innerHTML=(models||[]).slice(0,300).map(function(m){return '<option value="'+esc(m)+'">'}).join('');
    }
    window.detectModels=function(){
        var m=document.getElementById('addMsg');m.textContent='Detecting models…';m.className='set-msg';
        var body={provider:selectedAddProvider,key:document.getElementById('addKey').value.trim(),base_url:document.getElementById('addBase').value.trim()};
        fetch('/api/ai/models',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})
        .then(function(r){return r.json()}).then(function(d){
            if(d.error){m.textContent=d.error;m.className='set-msg err';return}
            fillModelHints(d.models);
            var n=(d.models||[]).length; m.textContent=n+' models found — click the Model box to pick.';m.className='set-msg ok';
            var am=document.getElementById('addModel'); if(n&&!am.value)am.value=d.models[0];
        }).catch(function(){m.textContent='Detection failed.';m.className='set-msg err'});
    };
    window.detectDefaultModels=function(){
        var p=document.getElementById('setProvider').value; if(!p)return;
        fetch('/api/ai/models',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider:p})})
        .then(function(r){return r.json()}).then(function(d){if(d.models&&d.models.length)fillModelHints(d.models)}).catch(function(){});
    };
